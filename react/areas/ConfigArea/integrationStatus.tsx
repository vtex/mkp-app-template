...
const IntegrationStatus: React.FC<IntegrationStatusProps> = ({ intl, config, setConfig, children }) => {
    return (
      <ToggleComponent
        tooltip={intl.formatMessage({ id: "admin/mkp-app.integrationStatus.tooltip" })}
        title={intl.formatMessage({ id: 'admin/mkp-app.integrationStatus.title' })}
        canEdit={true}
        id={'isIntegrationActive'}
        state={config.active}
        onOffValues={{
          on: intl.formatMessage({ id: 'admin/mkp-app.integrationStatus.value.on' }),
          off: intl.formatMessage({ id: 'admin/mkp-app.integrationStatus.value.off' }),
        }}
        children={ children }
        onChange={() => {
