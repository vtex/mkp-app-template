...
export const getConfig = async (ctx: Context) => {
  ...
  if(!headersExist(headers)){
    ...
    response.body = JSON.stringify({ message: "AppKey and AppToken are required" })
  } else if(!await vtexID.areValidAppKeyAndAppToken(APP_VENDOR, headers.appkey, headers.apptoken)){
    response.status = httpStatus.FORBIDDEN
    response.body = JSON.stringify({ message: "Invalid appKey and/or appToken" })
  } else {
